...
final class RecipeTests: XCTestCase {
  func test_parseRecipeWithName() {
    ...
    let recipe = Recipe(recipeText)

    guard let recipe = recipe else {
      XCTAssert(false, "Recipe initialization failed")
      return
    }
  ...
  func test_parseRecipe() {
    ...
    let recipe = Recipe(recipeText, name: "Insurmountable Wall")
  ...
  func test_operators() {
    ...
    guard let recipe = Recipe(recipeText) else {
      XCTAssert(false, "Recipe initialization failed")
      return
    }

    guard let genome = Parameters([279.64, 10.29, 35.95, 0.37, 0.49, 38.09, 0.32, 0.89]) else {
      XCTAssert(false, "Parameters initialization failed")
      return
    }
    let additionalGenome = Recipe.GenomeInfo(count: 49, area: nil, genome: genome)
