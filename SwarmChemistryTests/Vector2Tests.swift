...
final class Vector2RectTests: XCTestCase {
  func test_init() {
    let rectWithValue = Vector2.Rect(x: 1.0, y: 2.0, width: 10.0, height: 20.0)
    ...
    let rectWithInt = Vector2.Rect(x: 1, y: 2, width: 10, height: 20)
  ...
  func test_zero() {
    XCTAssert(Vector2.Rect.zero == Vector2.Rect(x: 0, y: 0, width: 0, height: 0))
  }

  func test_random() {
    let rect = Vector2.Rect(x: 100, y: 100, width: 100, height: 100)
  ...
  func test_contains() {
    let rect = Vector2.Rect(origin: .init(10, 10), size: .init(5, 5))
  ...
  func test_operators() {
    let rect = Vector2.Rect(x: 1, y: 2, width: 3, height: 4)

    XCTAssert(rect + Vector2.Rect(x: -1, y: -2, width: -3, height: -4) == .zero)
  ...
  func test_equality() {
    let rect = Vector2.Rect(x: 1.1, y: 2.2, width: 3.3, height: 4.4)

    XCTAssert(rect == Vector2.Rect(x: 1.1, y: 2.2, width: 3.3, height: 4.4))
    XCTAssert(rect != Vector2.Rect(x: 1.0, y: 2.2, width: 3.3, height: 4.4))
    XCTAssert(rect != Vector2.Rect(x: 1.1, y: 2.0, width: 3.3, height: 4.4))
    XCTAssert(rect != Vector2.Rect(x: 1.1, y: 2.2, width: 3.0, height: 4.4))
    XCTAssert(rect != Vector2.Rect(x: 1.1, y: 2.2, width: 3.3, height: 4.0))
  }

  func test_description() {
    let expectedDescription = "((x: 1.00, y: 2.00), (width: 3.00, height: 4.00))"

    let description1 = Vector2.Rect(x: 1, y: 2, width: 3, height: 4).description
    XCTAssert(description1 == expectedDescription)

    let description2 = Vector2.Rect(x: 1.001, y: 2.001, width: 3.001, height: 4.001).description
